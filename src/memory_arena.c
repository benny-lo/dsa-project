...
memory_arenas_manager_t *init_memory_arenas_manager(size_t arena_size) {
    memory_arenas_manager_t *manager = (memory_arenas_manager_t *)malloc(sizeof(memory_arenas_manager_t));
...
void *alloc(memory_arenas_manager_t *manager, size_t size) {
    ...
    if ((char *)manager->arenas_list->arena.current_ptr + size <= (char *)manager->arenas_list->arena.end_ptr) {
        void *ptr = manager->arenas_list->arena.current_ptr;
        manager->arenas_list->arena.current_ptr = (char *)manager->arenas_list->arena.current_ptr + size;
        return ptr;
    }

    memory_arenas_list_node_t *new_arenas_list_node = allocate_arenas_list_node(manager->arena_size);
...
memory_arenas_list_node_t *allocate_arenas_list_node(size_t arena_size) {
    memory_arenas_list_node_t *arenas_list_node = (memory_arenas_list_node_t *)malloc(sizeof(memory_arenas_list_node_t));
    arenas_list_node->arena.start_ptr = malloc(arena_size);
    arenas_list_node->arena.current_ptr = arenas_list_node->arena.start_ptr;
    arenas_list_node->arena.end_ptr = (char *)arenas_list_node->arena.start_ptr + arena_size;
